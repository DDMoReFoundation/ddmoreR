...
context("Loading in MDL into R objects, for fully populated MDL file with Design_Population")

jsonFile <- system.file("tests/data/json/Design_Population.json", package = "ddmore")
...
test_that("Expected mdlObj to have been created from the JSON-format text representing the MDL", {
	...
	expect_false(is.null(myMdlObj@MODEL_PREDICTION[[1]]$COMPARTMENT[[1]]), info="MODEL_PREDICTION::COMPARTMENT should be populated")
	expect_true(length(myMdlObj@MODEL_PREDICTION[[1]]$COMPARTMENT[[1]]) > 0, info="MODEL_PREDICTION::DEQ should be populated")
	expect_equal(myMdlObj@MODEL_PREDICTION[[1]]$.subtype, "BlockStmt", info="MODEL_PREDICTION::DEQ should be populated")
	expect_true(is.null(myMdlObj@MODEL_PREDICTION[[1]]$CC), info="MODEL_PREDICTION::COMPARTMENT should be populated")
...
test_that("Expected designObj to have been created from the JSON-format text representing the MDL", {
	...
	expect_false(is.null(myDesignObject@POPULATION), info="POPULATION slot should be populated")
...
jsonFileOutput <- file.path(tempdir(), "Design_Population.output.json")
