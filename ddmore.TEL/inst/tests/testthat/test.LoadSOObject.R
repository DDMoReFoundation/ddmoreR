...
test_that("Loading SOObject from latest hand coded PharmML SO fills expected slots", {
...
context("Loading SOObject from machine coded PharmML (run1.SO.xml)")

test_that("Loading SOObject from latest Machine Generated PharmML SO (run1.SO.xml)", {
  
  # Clear workspace. 
  rm(list=ls())
  
  machine.coded.data.path = system.file("tests/data/PharmMLSO/MachineGenerated/run1.SO.xml",  package = "DDMoRe.TEL")
