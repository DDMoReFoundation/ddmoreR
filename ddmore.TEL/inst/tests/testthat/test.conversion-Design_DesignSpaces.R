...
context("Loading in MDL into R objects, for fully populated MDL file with Design_DesignSpaces")

jsonFile <- system.file("tests/data/json/Design_DesignSpaces.json", package = "ddmore")
...
test_that("Expected designObj to have been created from the JSON-format text representing the MDL", {
	...
	expect_false(is.null(myDesignObject@DESIGN_SPACES), info="DESIGN_SPACES slot should be populated")
...
jsonFileOutput <- file.path(tempdir(), "Design_DesignSpaces.output.json")
