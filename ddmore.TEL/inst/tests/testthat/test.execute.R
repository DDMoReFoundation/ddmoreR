...
test_that("TEL.monitor with default import flags results in non-null result for successful job", {
  ...
  # when
  result = TEL.monitor(submission, importDirectory = "mock/path", server = serverMock, tel = telMock)
...
test_that("TEL.monitor with disabled import results in non-null result for successful job", {
  ...
  submission$status <- "Submitted"
  # when
  result = TEL.monitor(submission, importDirectory = "mock/path", importSO = FALSE, server = serverMock, tel = telMock)
...
test_that("TEL.monitor results in error for failed job", {
  ...
  #then
  expect_error(TEL.monitor(submission, importDirectory = "mock/path", server = serverMock, tel = telMock), info="Failed job should result in error.")
})
