...
test_that("A single update of an attribute value of a variable", {
	...
	updWarfParObj <- updateParObj(warfParObj, block="STRUCTURAL", item="POP_V", with=list(value=12.34))
...
test_that("An update of multiple attribute values of a variable", {
	...
	updWarfParObj <- updateParObj(warfParObj, block="VARIABILITY", item="CORR_PPV_CL_V", with=list(lo=-0.5, hi=+0.5))
	expect_equivalent(class(updWarfParObj), "parObj")
...
test_that("An update of values of the same attribute across multiple variables", {
	...
	updWarfParObj <- updateParObj(warfParObj, block="STRUCTURAL", item=structVarNames, with=list(value=varValues))
...
test_that("An update of values of multiple attribute across multiple variables", {
	...
	updWarfParObj <- updateParObj(warfParObj, block="STRUCTURAL", item=structVarNames, with=list(lo=varLos, value=varValues))
...
test_that("An update of values of a \"fix\" boolean attribute of a variable", {
	...
	updWarfParObj <- updateParObj(warfParObj, block="STRUCTURAL", item="BETA_V_WT", with=list(fix="false"))
...
test_that("Specifying a item to update that is not an item of the specified block, raises an error", {
	...
	expect_error(updateParObj(warfParObj, block="VARIABILITY", item=c("PPV_TLAG","POP_TLAG"), with=list(value=c(1.5, 2.5))),
...
test_that("Specifying an attribute to update that is not an attribute of all the specified items, raises an error", {
	...
	expect_error(updateParObj(warfParObj, block="VARIABILITY", item=c("PPV_TLAG","CORR_PPV_CL_V"), with=list(hi=2.0)),
