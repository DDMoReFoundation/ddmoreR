...
test_that("importJobResultFiles should import job result files", {
    ...
    stdErr <- file.path(jobWd, ".fis/stderr")
    writeLines("THIS IS STDERR FILE", stdErr)
    stdOut <- file.path(jobWd, ".fis/stdout")
    ...
    expect_true(file.exists(file.path(targetDirectory,"stdout")), info  = "STD OUT file should be created in the target directory.")
    expect_true(file.exists(file.path(targetDirectory,"stderr")), info  = "STD ERR file should be created in the target directory.")
